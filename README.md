...
>```shell
>   docker logs CONTAINERS-NAME-OR-ID
>```
...
> Запустить docker-composer.yaml
>```shell
>   docker-compose up
>```

> Запустить docker-composer.yaml в фоновом режиме
>```shell
>   docker-compose up -d
>```
...
1) Императивный подход: выше мы использовали команды docker, такие как docker stop, start, run exec и тд. То есть для каждого действия мы использовали специальную команду

2) Декларативный подход: в этом подходе описывается результат того, что мы хотим получить в **docker-compose.yaml** файле


#### **Формат YAML**

В этом формате есть списки и словари:
...
Yaml файлы обычно используются в качестве конфигурационных файлов
...
4) Автомотическое создание **изолированной сети** для взаимодействия контейнеров

5) Благодаря **DNS** возможно взаимодействие между контейнерами, используя **именя сервисов**

Пример docker-compose.yaml:

![Dockerfile](https://github.com/NemoZon/MDimages/raw/main/docker/dockercompose.png)

Здесь есть два сервиса: app и mongo, *имена сервисов выбираете ВЫ*. Для каждого сервиса докер создаст контейнер.

Для сервиса mongo указывается образ (image зарезервированное имя ключа и mongo название образа на DockerHub) и скачивается из DockerHub, если он ранее не был скачан.

Для сервиса app указывается создание (build зарезервированное имя ключа) кастомного образа на основании Dockerfile, который находится в папке ./app (Dockerfile нужно создать самому)

#### **Практика**

> В папке docker-compose-app создадим файл docker-compose.yaml и введем код из примера, там же создадим папку app, в которой будет два файла Dockerfile и main.py.

> В main.py будет написан код для общения с базой данных mongoDB, который мы создадим позже. Этот файл будет нуждаться в библиотеке MongoClient, который по умолчанию не установлен в образе python

> В Dockerfile пишем тот же код, что и при первом создании кастомного образа python добавив одну инструкцию: RUN pip install pymongo, таким образом в нашем образе будет установлена библиотека для импорта
...
> Убедимся, что main.py отработал и вывел список баз данных в логи
...
> Перебилдим приложение
>```shell
>   docker-compose up -d --build
>```

> Посмотрим логи контейнера app (у меня docker-compose-app_app_1)
>```shell
>   docker logs 98cbd97ef781
>```